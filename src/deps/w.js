...
W = {
  ...
  setState: (state, type, texture, i, normal = [], A, B, C, Ai, Bi, Ci, AB, BC) => {
    ...
    if (state.fov) {
      W.projection = new DOMMatrix([
        1 / Math.tan((state.fov * Math.PI) / 180) / (W.canvas.width / W.canvas.height),
        0,
        0,
        0,
        0,
  ...
  draw: (now, dt, v, i, transparent = []) => {
    ...
    W.gl.uniformMatrix4fv(W.gl.getUniformLocation(W.program, "eye"), false, v.toFloat32Array());
    ...
    W.gl.uniformMatrix4fv(W.gl.getUniformLocation(W.program, "pv"), false, v.toFloat32Array());
  ...
  render: (object, dt, just_compute = ["camera", "light", "group"].includes(object.type), buffer) => {
    ...
    if (!just_compute) {
      ...
      W.gl.vertexAttrib4fv(W.gl.getAttribLocation(W.program, "col"), W.col(object.b));
  ...
  lerp: (item, property) =>
    W.next[item]?.a
      ? W.current[item][property] +
        (W.next[item][property] - W.current[item][property]) * (W.next[item].f / W.next[item].a)
...
W.smooth = (state, dict = {}, vertices = [], iterate, iterateSwitch, i, j, A, B, C, Ai, Bi, Ci, normal) => {
  ...
  for (i = 0; i < iterate.length * 2; i += 3) {
    ...
    normal =
      i > j ? [0, 0, 0] : [AB[1] * BC[2] - AB[2] * BC[1], AB[2] * BC[0] - AB[0] * BC[2], AB[0] * BC[1] - AB[1] * BC[0]];
    dict[A[0] + "_" + A[1] + "_" + A[2]] ||= [0, 0, 0];
    dict[B[0] + "_" + B[1] + "_" + B[2]] ||= [0, 0, 0];
    dict[C[0] + "_" + C[1] + "_" + C[2]] ||= [0, 0, 0];
    W.models[state.type].normals[Ai] = dict[A[0] + "_" + A[1] + "_" + A[2]] = dict[A[0] + "_" + A[1] + "_" + A[2]].map(
      (a, i) => a + normal[i],
    );
    W.models[state.type].normals[Bi] = dict[B[0] + "_" + B[1] + "_" + B[2]] = dict[B[0] + "_" + B[1] + "_" + B[2]].map(
      (a, i) => a + normal[i],
    );
    W.models[state.type].normals[Ci] = dict[C[0] + "_" + C[1] + "_" + C[2]] = dict[C[0] + "_" + C[1] + "_" + C[2]].map(
      (a, i) => a + normal[i],
    );
  }
};
