/* CREDIT TO: curtastic [https://github.com/curtastic] */
...
!(function () {
  var buffers = {},
    ...
    pianoify = (note) => b(note, b(note, 0) ** 2 + b(note, 0.25) * 0.75 + b(note, 0.5) * 0.1);
  ...
  var makeNote = (note, seconds, sampleRate) => {
    ...
    if (!buffer) {
      ...
      var sampleTotal = (sampleRate * seconds) | 0,
        sampleRest = sampleRate * (seconds - 0.002),
        bufferArray,
        i;
        ...
        bufferArray[i] =
          ...
          (i < 88
            ...
            : // The other samples represent the rest of the note
              (1 - (i - 88.2) / sampleRest) ** ((Math.log(1e4 * note) / 2) ** 2)) * pianoify(i * note);
      }
    }
    return buffer;
  };
